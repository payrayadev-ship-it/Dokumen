...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // Only allow POST requests for doc comparison
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Metode tidak diizinkan. Gunakan POST." });
  }

  const { docAName, docBName, docAText, docBText, category } = req.body || {};

  if (!docAText || !docBText) {
    return res.status(400).json({ error: "Teks Dokumen A dan B harus disediakan." });
  }

  // Fallback Mock Data generation if no Gemini API Key is configured in Vercel environment
  if (!hasGeminiKey()) {
    console.warn("GEMINI_API_KEY is not configured or placeholder in Vercel. Running in Mock Mode.");
...
Tolong lakukan ekstraksi perbedaan se-detail mungkin dan tentukan level risiko (riskLevel) secara proporsional.`;
    ...
    console.error("Gemini API Error on Vercel Node Serverless:", error);
